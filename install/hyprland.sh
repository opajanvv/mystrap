...
log "Updating hyprpm..."
sudo hyprpm update || warn "Failed to update hyprpm"

# Check if plugin is already added
PLUGIN_URL="https://github.com/Duckonaut/split-monitor-workspaces"
if sudo hyprpm list | grep -q "split-monitor-workspaces"; then
    log "Plugin already installed"
else
    log "Installing split-monitor-workspaces plugin..."
    sudo hyprpm add "$PLUGIN_URL" || die "Failed to install plugin"
fi

# Enable plugin
log "Enabling split-monitor-workspaces plugin..."
sudo hyprpm enable split-monitor-workspaces || warn "Failed to enable plugin"
